...
Reads a metadata.csv created from the filenames with columns:
  filename, method, location, sample, mode, magnification, magnification_value, ext, abs_path
...
def filtered_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sel_methods = c1.multiselect("Method", methods, default=methods)
    sel_locations = c2.multiselect("Location", locations, default=locations)
    ...
    st.sidebar.markdown("**Magnification**")

    sel_mags_text = st.sidebar.multiselect("Labels", mags_text, default=mags_text)

    df = df[df["magnification"].isin(sel_mags_text)]

    if df["magnification_value"].notna().any():
        min_mag = float(df["magnification_value"].min(skipna=True))
        max_mag = float(df["magnification_value"].max(skipna=True))
        if min_mag <= max_mag:
            ...
            df = df[
                df["magnification_value"].isna()
                | (
                    (df["magnification_value"] >= lo)
                    & (df["magnification_value"] <= hi)
                )
            ]
...
def main():
    ...
    src_mode = st.sidebar.radio("Data source", ["From folder", "Upload CSV"])
    csv_path = None
    ...
    st.sidebar.download_button(
        ...
        file_name="metadata_filtered.csv",
        mime="text/csv",
        disabled=df.empty,
    )
    ...
    group_axis = st.sidebar.selectbox(
        ...
        ["method", "magnification", "mode"],
        index=0 if view_mode == "By Sample" else 0,
    )
    images_per_row = st.sidebar.slider("Images per row", 1, 8, 2)
